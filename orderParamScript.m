...
%     expNaiveAbs = findInnerProduct(exp_abs, naive_abs);
    ...
    expModelPolar = norm(exp_polar - model_polar);
    expNaivePolar = norm(exp_polar - naive_polar);
    
    expModelAng = norm(exp_ang - model_ang);
    expNaiveAng = norm(exp_ang - naive_ang);
    
    expModelAbs = norm(exp_abs - model_abs);
    expNaiveAbs = norm(exp_abs - naive_abs);

    comparisonPolar(runnum, :) = [expModelPolar, expNaivePolar];
    comparisonAng(runnum, :) = [expModelAng, expNaiveAng];
    comparisonAbs(runnum, :) = [expModelAbs, expNaiveAbs];
end

polarName = strcat('comparisonPolarNorm', num2str(expnum), '.png');
angName = strcat('comparisonAngNorm', num2str(expnum), '.png');
absName = strcat('comparisonAbsNorm', num2str(expnum), '.png');
